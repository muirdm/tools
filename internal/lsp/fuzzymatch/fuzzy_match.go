...
const (
	...
	// fuzzyPrefixMatch like fuzzyConsecMatch but only for matches anchored at
...
// means better match). In order to match, each input rune must appear in order
// in the target string. Uppercase input runes must match exactly with target
// runes, but lowercase input runes match case insensitively. Matching is greedy,
// so the first target rune that matches an input rune is always taken. For
// short inputs, the score will typically be in the range [0, 10].
func Match(input, target string) (bool, int) {
	...
	for targetByteIdx, targetRune := range target {
		...
		if match {
			...
			// Matches the start of a word within an identifer.
			if targetByteIdx > 0 {
				switch {
